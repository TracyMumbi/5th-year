...
function Distance() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (distance.trim() !== '') {
      try {
        ...
        if (!response.ok) {
          throw new Error('Failed to submit distance');
        }
        setSubmitteddistance(distance);
        setdistance('');
        fetchdistanceData();
      } catch (error) {
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto bg-white shadow-md rounded-lg mt-8">
      <h2 className="text-2xl font-bold mb-6 text-blue-600 text-center">Car Distance Over Time Tracker</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 mb-8">
        <textarea
          ...
          placeholder="Enter current car distance (e.g., 120 cm)"
          value={distance}
      ...
      {submitteddistance && (
        <div className="mb-10 bg-blue-50 border-l-4 border-blue-500 text-blue-700 p-4 rounded">
          <p className="font-semibold">Submitted distance:</p>
          <p>{submitteddistance}</p>
        </div>
      )}

      <h3 className="text-xl font-semibold mb-4 text-gray-700">Distance Over Time</h3>
      ...
      <div className="h-80 w-full">
        {loading ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={distanceData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
              <defs>
              ...
              <YAxis
                label={{ value: 'Distance (cm)', angle: -90, position: 'insideLeft', dx: -10 }}
              />
